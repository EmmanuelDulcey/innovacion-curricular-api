...
- El sistema se construye por **versiones incrementales** (v1, v2, v3, v4),
  cada una con su spec kit propio (documentos 2 a 9: API y Frontend).  
...
- **No se anticipa**: nada de una versión futura se construye antes de tiempo
  (ej. JWT en v1, tablas con FK en v1).  
- El repositorio siempre contiene la **versión en curso funcionando**.

---

## Artículo 2 — Stack: C# + ASP.NET Core + Angular

- API: lenguaje **C#** sobre **ASP.NET Core**.  
- Acceso a datos con **Dapper** (SQL escrito a mano, parametrizado).  
- Paquetes externos permitidos desde la v1:  
  - `Microsoft.Data.SqlClient`  
  - `Dapper`  
  - `Swashbuckle.AspNetCore` (Swagger).  
- Frontend: **Angular** (TypeScript) con Angular CLI, presente **desde la v1**.  
  Se usa `HttpClient` para consumir la API en `http://localhost:8036`
...
**SQL Server + API + Frontend** en un solo comando.  
El código de la API corre con `dotnet watch`; el frontend con `ng serve`;
guardar un archivo recompila y refresca.
...
Lo que crece por versiones es la **API y el Frontend**.  
...
- La interfaz del Frontend puede mostrar textos en español; identificadores y
...
| Cosa | Convención |
|---|---|
| Puertos del proyecto | API **8036** · SQL Server **11467** · Frontend **8037** |
| Rutas API | `/swagger` · `/api/{tabla}` |
| Rutas Frontend | `/` (dashboard) · `/tablas/{tabla}` (CRUD visual) |
| Nombres API | PascalCase en inglés; interfaces con prefijo `I`; carpetas `Controllers/ Models/ Requests/ Services/ Repositories/ Exceptions/ Tests/` |
| Nombres Frontend | `camelCase`; carpetas `frontend/src/app/` con `pages/`, `components/`, `services/`, `models/` |
...
- **Monorepo**: un único repositorio **privado**; todo vive en el mismo
  proyecto y el backend y el Frontend están separados por carpetas:
  `api_innovacion/` y `frontend/` (ver `3_plan.md`). Conceptualmente la API
  y el Frontend son dos módulos desacoplados, pero comparten el mismo repo,
  los mismos commits y los mismos tags.  
...
| Versión | API (backend) | Frontend (Angular) | Cierre |
|---|---|---|---|
| **v1** | CRUD de las **7 tablas sin FK**: `area_conocimiento`, `universidad`, `aspecto_normativo`, `practica_estrategia`, `enfoque`, `car_innovacion`, `aliado`. Catálogos del Excel cargados. | Dashboard con métricas de los 7 catálogos; CRUD visual (listar, crear, editar, eliminar) de cada tabla. | Los 7 CRUD funcionan de punta a punta (API + Frontend); borrado lógico filtrando inactivos; tag `v1`. |
| **v2** | CRUD de las **15 tablas con FK**: `facultad`, `programa`, `acreditacion`, `registro_calificado`, `activ_academica`, `pasantia`, `premio`, `docente_departamento`, `alianza` y tablas puente (`programa_ac`, `programa_pe`, `programa_ci`, `an_programa`, `enfoque_rc`, `aa_rc`). | CRUD visual de las 15 tablas con selects que resuelven las FK; dashboard ampliado con métricas adicionales. | Los 22 CRUD funcionan desde el navegador; validación de integridad referencial; tag `v2`. |
| **v3** | Gestión de usuarios: `POST /api/login` con JWT; middleware de autenticación y autorización; CRUD de `usuario`, `rol`, `rol_usuario` solo para administradores; logout. | Login; menú diferenciado por roles (consulta vs admin); guardias de ruta Angular; vistas admin de usuarios/roles. | Un usuario "consulta" no puede escribir; solo el admin ve usuarios/roles; contraseñas hasheadas; tag `v3`. |
| **v4** | 10 consultas multitabla (mínimo 4 tablas cada una). | Dashboard con gráficos (Chart.js o similar); páginas corporativas; responsive/PWA; deploy. | Sitio publicado y funcional con secretos en variables de entorno del servidor; tag `v4`. |

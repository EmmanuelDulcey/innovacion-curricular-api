...
{
    private readonly IServicioAspectoNormativo _servicio;

    public AspectoNormativoController(IServicioAspectoNormativo servicio)
    {
        _servicio = servicio;
    }
    ...
    public async Task<IActionResult> Listar([FromQuery] int limite = 1000)
    {
        ...
        try
        {
            ...
            return Ok(new RespuestaLista<AspectoNormativo>
            {
    ...
    [HttpPost]
    public async Task<IActionResult> Crear([FromBody] AspectoNormativoCrear peticion)
    {
        try
        {
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Reemplazar(int id, [FromBody] AspectoNormativoReemplazo peticion)
    {
        try
        {
    ...
    [HttpPatch("{id:int}")]
    public async Task<IActionResult> Actualizar(int id, [FromBody] AspectoNormativoActualizar peticion)
    {
        try
        {
            return Ok(new RespuestaFilasAfectadas
